...
Instead of Pierre making paying a hefty transaction fee by making a transaction on the main Bitcoin network, he can pay minimal fees by using the LN. It allows for many instant payments to occur directly between himself and the boulangerie which are then grouped and added to the main Bitcoin blockchain as single transactions.

### Smart Contracts

The LN scales by using [smart contracts](https://river.com/learn/terms/h/hashed-time-locked-contract-htlc/) to create a network of channels by allocating payments through other open channels. For example, Pierre could buy a croissant through Marie if she already has an open channel with Charles, who has an open channel with the boulangerie. This ensures the network operates in a decentralised fashion and removes the need for trust between the parties involved.
